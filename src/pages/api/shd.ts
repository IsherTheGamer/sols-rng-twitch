...
import { attemptShdCraft, attemptShdUpgrade, formatShdStatus } from "@/lib/core-system";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (action === "craft") return text(res, await attemptShdCraft(channelId, user, totalRolls));
  if (action === "upgrade") return text(res, await attemptShdUpgrade(channelId, user));
