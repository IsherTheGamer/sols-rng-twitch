...
const PATH_WALL_COMPONENTS: Record<CorePath, readonly string[]> = {
...
type WallRecipePath = Exclude<CorePath, "universal">;
...
export async function grantCoreMaterials(
  channelId: string,
  user: NightbotUser | null,
  materials: Record<string, number>
): Promise<void> {
...
export async function formatQuestStatus(
  ...
  const rewardMultiplier = getPathModifiers(state).questRewardMultiplier;
