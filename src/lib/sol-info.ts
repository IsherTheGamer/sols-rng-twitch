...
import { INFO_TOPICS, resolveTextAlias } from "./command-aliases";
import { aliasSuggestionText } from "./fuzzy-alias";
...
function auraInfo(q:string,p?:string){
  if(q&&!/^\d+$/.test(q)){const r=resolveAuraByQuery(q);if(r.status!=="matched")return aliasSuggestionText(r,"aura");const a=r.match.value;const flags=[a.biome?`Biome ${title(a.biome)}`:null,a.event?`Event ${title(a.event)}`:null,a.devBiome?`Dev ${title(a.devBiome)}`:null,a.potion?`Potion ${title(a.potion.id)} ${rarity(a.potion.rarity)}`:null,a.luckImmune?"Raw luck only":null,a.unobtainable?"Unobtainable":null,a.deleted?"Deleted":null].filter(Boolean);return truncate(`✨ ${a.name} | ${rarity(a.rarity)}${flags.length?` | ${flags.join(" | ")}`:""}`,390);}
  return paginate([...auras].sort((a,b)=>a.rarity-b.rarity),/^\d+$/.test(q)?q:p,a=>`${a.name} ${rarity(a.rarity)}`,"✨ Auras");
}
function biomeInfo(q:string,p?:string){
  if(q&&!/^\d+$/.test(q)){const r=resolveBiome(q);if(r.status!=="matched")return aliasSuggestionText(r,"biome");const b=r.match.value;const chance=b.spawnPerSecond?`Spawn/sec 1/${Math.round(1/b.spawnPerSecond).toLocaleString("en-US")}`:b.spawnOnChange?`On-change 1/${Math.round(1/b.spawnOnChange).toLocaleString("en-US")}`:b.deviceChance?`Device 1/${b.deviceChance.toLocaleString("en-US")}`:b.manualOnly||b.devOnly?"Manual/dev only":"Normal pool";return truncate(`🌍 ${b.name} | ${chance} | BT x${b.breakthroughMultiplier}${b.isRareBiome?" | Rare":""}`,390);}
  return paginate(biomes,/^\d+$/.test(q)?q:p,b=>`${b.name}${b.isRareBiome?" rare":""}`,"🌍 Biomes");
}
function potionInfo(q:string,p?:string){
  if(q&&!/^\d+$/.test(q)){const r=resolvePotion(q);if(r.status!=="matched")return aliasSuggestionText(r,"potion");const x=r.match.value;return truncate(`🧪 ${x.name} | +${Math.floor(x.luck).toLocaleString("en-US")} luck${x.clearsBuffs?" | Clears buffs":""}${x.requiresEvent?` | Event ${title(x.requiresEvent)}`:""}${x.exclusiveAuras?.length?` | ${x.exclusiveAuras.length} exclusive aura(s)`:""}`,390);}
  return paginate(potions,/^\d+$/.test(q)?q:p,x=>`${x.name} +${Math.floor(x.luck).toLocaleString("en-US")}`,"🧪 Potions");
}
function rollTokens(kind:string,p?:string){const mode=kind||"boosts",all=getAllTokenDefinitions(),special=(t:(typeof all)[number])=>Boolean((t.flatLuck??0)>0||(t.rareBiomePercentLuck??0)>0||(t.finalLuckMultiplier??1)>1),list=mode.includes("potion")||mode.includes("roll")?all.filter(t=>t.kind==="potion"):mode.includes("special")?all.filter(t=>t.kind==="percent_luck"&&special(t)):all.filter(t=>t.kind==="percent_luck"&&!special(t));return formatTokenDefinitionPage(list,p,mode.includes("potion")?"🧪 Roll/Potion Tokens":mode.includes("special")?"✨ Special Luck Tokens":"🎟️ Luck Boost Tokens");}

export function formatSolInfo(rawQuery:string){
  const parts=rawQuery.trim().split(/\s+/).filter(Boolean);if((parts[0]??"").replace(/^!+/,"").toLowerCase()==="sol")parts.shift();const rawTopic=parts.shift()??"help",resolved=resolveTextAlias(rawTopic,INFO_TOPICS,"info topic"),{query,page:p}=split(parts),pq=p||query;
  if(!resolved.value){const aura=resolveAuraByQuery([rawTopic,query].filter(Boolean).join(" "));return aura.status==="matched"?auraInfo([rawTopic,query].filter(Boolean).join(" ")):resolved.error??"Unknown info topic.";}
  const topic=resolved.value;
  if(topic==="help")return"📘 Help | !info start | currencies | paths | token <name> | obtain <item> | research | relics | blueprints";
  if(topic==="start")return staticGuide(START,pq,"🚀 Start Guide");
  if(topic==="currencies")return staticGuide(CURRENCIES,pq,"💰 Currency Guide");
  if(topic==="commands")return staticGuide(COMMANDS,pq,"🤖 Commands");
  if(topic==="activity")return staticGuide(ACTIVITY,pq,"🧠 Activity Guide");
  if(topic==="research")return staticGuide(RESEARCH,pq,"🧠 Research Guide");
  if(topic==="relics")return staticGuide(RELICS,pq,"🧿 Relic Guide");
  if(topic==="blueprints")return staticGuide(BLUEPRINTS,pq,"📘 Blueprint Guide");
  if(topic==="auras")return auraInfo(query,p);
  if(topic==="biomes")return biomeInfo(query,p);
  if(topic==="potions")return potionInfo(query,p);
  if(topic==="tokens"){const first=(query.split(/\s+/)[0]??"").toLowerCase();if(["boost","boosts","potion","potions","special"].includes(first))return rollTokens(first,p);if(["source","sources","obtain","how","get"].includes(first))return formatTokenSourceGuide(query.split(/\s+/).slice(1).join(" "),p);if(isKnownTokenGuide(query))return formatTokenSourceGuide(query,p);return"🎟️ Two token systems | Core tokens: !info token quest | Roll tokens: !info token boosts/potions | Owned Core tokens: !core tokens";}
  if(topic==="boosts")return rollTokens("boosts",query||p);
  if(topic==="special")return rollTokens("special",query||p);
  if(topic==="paths")return formatPathComponentGuide(query,p);
  if(topic==="materials")return formatMaterialGuide(query,p);
  if(topic==="components")return formatComponentGuide(query,p);
  if(topic==="boxes")return formatBoxGuide(query,p);
  if(topic==="obtain")return formatObtainGuide(query,p);
  if(topic==="events")return paginate(events as Array<{name:string}>,pq,e=>e.name,"🎉 Events");
  if(topic==="dev")return paginate(devEvents as Array<{name?:string;id:string}>,pq,e=>e.name??title(e.id),"🛠️ Dev Biomes");
  if(topic==="devices")return paginate(devices as Array<{name:string}>,pq,d=>d.name,"📟 Devices");
  if(["core","shd","reactor","quests"].includes(topic))return truncate(`📘 ${title(topic)} | Status: !${topic==="quests"?"pquests":topic} | Exact source: !info obtain <name> | Token help: !core tokens`,390);
