...
export async function addDynamicRollAccess(options: {
  ...
  if (!username) throw new Error("Enter a valid Twitch username.");
  ...
  if (entries.length >= 100) {
    throw new Error("Dynamic allowlist is limited to 100 users.");
  }

  entries.push({
    username,
    addedById: options.addedBy?.providerId ?? "cron-token",
    addedByName: options.addedBy?.displayName ?? options.addedBy?.name ?? "cron-token",
    addedAt: Date.now(),
  });
