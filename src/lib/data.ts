...
export const STATUS_INTERVAL_SECONDS = biomesData.statusIntervalSeconds as number;
export const NORMAL_POOL_WEIGHTS = biomesData.normalPoolWeights as Array<{ id: string; weight: number }>;
...
export function findBiome(id: string): BiomeDef | undefined {
  ...
  return biomeMap.get(norm) ?? biomes.find((b) => b.id === norm || b.name.toLowerCase() === id.toLowerCase());
}

export function findAuraByQuery(query: string): AuraDef | undefined {
  const q = query.toLowerCase().trim();
  const asId = q.replace(/[:\s]+/g, "_").replace(/_+/g, "_").replace(/^_|_$/g, "");
  return auras.find(
    (a) =>
      a.id === asId ||
      a.id === q.replace(/\s+/g, "_") ||
      a.name.toLowerCase() === q ||
      a.name.toLowerCase().replace(/[^a-z0-9]/gi, "") === q.replace(/[^a-z0-9]/gi, "")
  );
}
