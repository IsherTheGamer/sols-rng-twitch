...
const POST_100_DEV_LOOTBOX_CHANCE = 0.1;
...
const LOOT_POOLS: Record<LootPoolId, LootPoolEntry[]> = {
  ...
  dev: [
    { id: "axis_potion", name: "Token of Axis Potion", minAmount: 1, maxAmount: 1, weight: 25 },
    { id: "xyz_potion", name: "Token of XYZ Potion", minAmount: 1, maxAmount: 1, weight: 25 },
    { id: "word_potion", name: "Token of Word Potion", minAmount: 1, maxAmount: 1, weight: 25 },
    { id: "chaos_potion", name: "Token of Chaos Potion", minAmount: 1, maxAmount: 1, weight: 25 },
  ],
};
...
function addGroupedReward(
  grouped: Map<string, { id: string; name: string; amount: number }>,
  entry: LootPoolEntry
): void {
...
function resolveRandomLootReward(reward: LevelReward): LevelReward[] {
  ...
  for (let i = 0; i < rolls; i++) {
    addGroupedReward(grouped, rollWeighted(pool));
  }

  if (poolId === "post100" && Math.random() < POST_100_DEV_LOOTBOX_CHANCE) {
    addGroupedReward(grouped, rollWeighted(LOOT_POOLS.dev));
  }
